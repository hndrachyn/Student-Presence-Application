...
root = Tk()
root.title("OOP TKINTER")  # Title Program
...
# komponen komponen entry
        ...
        tombol_report = Button(self.window, text='report', bg='#bcbabe', fg='#011a27',
                               command=self.load_data, width=5, height=2)
        self.jam()
...
# database mahasiswa
    ...
    def database(self):
        ...
        self.umur.set('')
        self.akun.set('')
        if angka2 in self.akun_unesa:
            if angka2 in self.fullname:
                self.nama_lengkap.set(self.fullname[angka2])
                self.akun.set(self.akun_unesa[angka2])
        elif angka1 == '':
...
# fungsi tombol tombol
    def tombolsubmit(self):
        ...
        messagebox.showinfo('info', 'thankyou ❤️')
    ...
    def tombolclear(self):
        ...
        self.akun.set('')
        self.matakuliah_combobox.set('select')
        self.capture_image.config(image=new_photo)
        self.capture_image.image = new_photo
    ...
    def print_pdf(self):
        ...
        pdf.cell(
            200, 10, txt=f'Nama:\t{self.nama_lengkap.get()}', ln=1, align="L")
        pdf.cell(200, 10, txt=f'NIM:\t{self.nim.get()}', ln=2, align="L")
        pdf.cell(
            200, 10, txt=f'Akun UNESA:\t{self.akun.get()}', ln=3, align="L")
        pdf.cell(
            200, 10, txt=f'Alamat:\t{self.locname}', ln=4, align="L")
        pdf.image(
            f'db\{self.nama_lengkap.get()}_{self.waktu}.jpg', 30, 70, 150, 150)
        pdf.output(f"{self.nama_lengkap.get()}_{self.waktu}.pdf")
    ...
    def add_camera(self):
        img = Image.open('db/'+f'{self.nama_lengkap.get()}_{self.waktu}.jpg')
        resize_image = img.resize((130, 130))
        img2 = ImageTk.PhotoImage(resize_image)
        self.capture_image.config(image=img2)
        self.capture_image.image = img2
